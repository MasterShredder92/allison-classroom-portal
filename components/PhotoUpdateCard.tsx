...
export default function PhotoUpdateCard({ photo }: { photo: PhotoUpdate }) {
  ...
  return (
    <article className="bulletin-card pop-card group overflow-hidden rounded-[1.9rem] bg-white pt-8" style={{ '--tilt': '1deg' } as CSSProperties}>
      <div className="relative z-10 mx-4 aspect-square overflow-hidden rounded-[1.35rem] bg-accent-sky-blue/18 shadow-inner">
      ...
      <div className="relative z-10 border-t-[6px] border-accent-light-pink p-6">
        <p className="mb-2 text-xs font-black uppercase tracking-[0.18em] text-neutral-dark-gray">{formattedDate}</p>
        {photo.title && <h3 className="font-serif text-2xl font-black leading-tight text-neutral-text transition-colors group-hover:text-accent-pink">{photo.title}</h3>}
